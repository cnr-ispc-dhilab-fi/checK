// ATON/wapps/checK/js/project-setup/client-setup.js
// Project-setup specific client — requires check-client.js
...
async function upload3DAssetForCurrentProject() {
  ...
  try {
    ...
    const patch = { [assetEntry.id]: assetEntry };
...
async function delete3DAsset(projectId, assetId) {
  ...
  try {
    const storageId = getProject3DAssetsStorageId(projectId);
    const allAssets = (await ATON.App.getStorage(storageId)) || {};
    ...
    const res = await fetch(
      `${SERVER_BASE}/projects/${projectId}/files`,
      {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ glbFilename, thumbFilename })
      }
    );
          ...
          [referenceGM]: {
            phase: {
              [phaseNo]: {
                ...
                sceneID: currentPhaseSceneID || null,
                analytics: {
...
async function addPhaseToConfig(phaseNo) {
  ...
  let patch = {
    [referenceGM]: {
      phase: { [phaseNo]: { name: `Phase ${phaseNo}` } }
    }
  };
    ...
    for (let key of sortedKeys) {
      if (key > phaseNo) {
        ...
        await ATON.App.addToStorage(storageId, {
          [referenceGM]: { phase: { [newKey]: phaseData } }
        });
      }
    }
...
// ===============================
// PROTOCOL ASSET UPLOAD
// ===============================

let currentUploadedAsset = null;
...
async function addAssetToLibrary(projectId, assetData) {
  ...
  const asset = { ...assetData, type: type };
  const patch = { [assetData.id]: asset };
...
async function deleteAsset(projectId, assetId) {
  ...
  try {
    ...
    if (!asset) throw new Error(`Asset ${assetId} not found in library`);
    ...
    if (Object.keys(filesToDelete).length > 0) {
      ...
      if (!response.ok) console.warn("Warning: Could not delete physical files");
    }
